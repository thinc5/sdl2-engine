...
/**
 * Add a new entity to the manager if there is space, if not allocate more space for
 * new entities.
 */
void add_entity(EntityManager* entityManager, Entity (*init_entity)(void), SDL_Rect rect);

/**
 * Remove all entities that are marked to delete then shift all right of the removed
 * entity left to conserve space.
 */
